    ...
    <div className="flex h-screen w-full bg-[#0b0e14] overflow-hidden text-on-surface font-body">
      ...
      <aside className="hidden md:flex flex-col w-[260px] bg-surface-low border-r-[0.5px] border-outline-variant/30 flex-shrink-0">
        <div className="p-6 border-b-[0.5px] border-outline-variant/30">
          <h2 className="font-display text-primary font-bold text-lg tracking-[0.1rem]">LAB_DE_DADOS_V1.0</h2>
        </div>
        ...
        <nav className="flex flex-col mt-4 gap-1">
          {navLinks.map((link) => (
            ...
            >
              <link.icon size={16} />
              <span className="font-display text-xs tracking-[0.1rem] mt-1">{link.label}</span>
            </Link>
          ))}
        </nav>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-[#0b0e14] flex flex-col p-8 md:hidden">
          <div className="flex justify-between items-center mb-12">
            <h2 className="font-display text-primary font-bold text-lg tracking-[0.1rem]">MENU_LAB</h2>
            <button onClick={() => setIsMobileMenuOpen(false)} className="text-primary p-2 border border-primary/20">
              <X size={24} />
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col relative h-full overflow-hidden">
        ...
        <header className="flex justify-between items-center w-full px-6 md:px-8 py-4 md:py-6 border-b-[0.5px] border-outline-variant/20 bg-[#0b0e14]/80 backdrop-blur-md z-40">
          <div className="flex items-center gap-4">
            ...
            <h2 className="md:hidden font-display text-primary font-bold text-sm tracking-[0.1rem]">LAB_DE_DADOS</h2>
          </div>
          
          <div className="flex gap-4 text-primary">
            <Settings size={18} className="cursor-pointer hover:text-primary-container transition-colors hidden sm:block" />
            <Bell size={18} className="cursor-pointer hover:text-primary-container transition-colors hidden sm:block" />
        ...
        <main className="flex-1 overflow-y-auto scroll-smooth">
          ...
          {/* STANDARD FOOTER */}
          <footer className="max-w-7xl mx-auto w-full px-8 md:px-12 mt-auto">
            <div className="w-full flex flex-col md:flex-row justify-between items-center gap-4 py-8 border-t-[0.5px] border-outline-variant/30 font-display text-[0.55rem] text-on-surface-variant tracking-[0.2rem] uppercase">
               <div>© 2024 LAB_INFINITO // ACESSO_RESTRITO</div>
               <div className="flex flex-wrap justify-center gap-6">
                 <span>TERMINAL</span>
                 <span>REPOSITÓRIO</span>
                 <span>COMUNICAÇÃO</span>
                 <span className="text-primary hidden lg:inline-block">COORD: 42.06.12 // 000MB_Y: 10.00.04</span>
               </div>
            </div>
          </footer>
        </main>
      ...
      <DataScrubber current={getScrubberCurrent()} total={5} />
    </div>
  );
}
