...
import { Mail, Briefcase, MessageCircle, ArrowRight, Zap, Loader2, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validação básica: garante que todos os campos estão preenchidos
    ...
    <div className="p-8 md:p-12 pb-32 flex flex-col max-w-7xl mx-auto w-full relative">
      <header className="mb-12">
         <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-display text-[0.55rem] text-tertiary-container uppercase tracking-[0.2rem]">ACESSO_AUTORIZADO // SETOR 7</span>
         </div>
         <h1 className="font-display text-5xl md:text-7xl text-on-surface font-bold tracking-[-0.02em] leading-tight">
           Central de <br /> 
           <span className="bg-gradient-to-r from-tertiary-container to-tertiary text-transparent bg-clip-text drop-shadow-[0_0_20px_rgba(0,251,64,0.3)]">Comunicação</span>
         </h1>
      </header>
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 w-full">
         ...
         <section className="flex-1 flex flex-col gap-6 order-2 lg:order-1">
            ...
            <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
               <div className="flex flex-col gap-2">
                  <label className="font-display text-[0.55rem] text-on-surface-variant uppercase tracking-[0.15rem]">IDENTIFICAÇÃO</label>
                  <input 
                    type="text" 
               ...
               <div className="flex flex-col gap-2">
                  <label className="font-display text-[0.55rem] text-on-surface-variant uppercase tracking-[0.15rem]">SINAL_DE_RETORNO (E-MAIL)</label>
                  <input 
                    type="email" 
               ...
               <div className="flex flex-col gap-2">
                  <label className="font-display text-[0.55rem] text-on-surface-variant uppercase tracking-[0.15rem]">FREQUÊNCIA DO SINAL (ASSUNTO)</label>
                  <input 
                    type="text" 
               ...
               <div className="flex flex-col gap-2">
                  <label className="font-display text-[0.55rem] text-on-surface-variant uppercase tracking-[0.15rem]">DADOS DA MENSAGEM</label>
                  <textarea 
                    rows={5}
               ...
               <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-2">
                  ...
                  <div className="flex-1">
                    {status === 'success' && (
                      <div className="flex items-center gap-2 text-tertiary-container animate-in fade-in slide-in-from-left-2">
                        ...
                        <span className="font-display text-[0.65rem] uppercase tracking-[0.1rem]">Transmissão concluída com sucesso!</span>
                      </div>
                    )}
                    {status === 'error' && (
                      <div className="flex items-center gap-2 text-secondary-container animate-in fade-in slide-in-from-left-2">
                        <AlertTriangle size={18} />
                        <span className="font-display text-[0.65rem] uppercase tracking-[0.1rem]">Falha na transmissão. Tente novamente.</span>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    <span className="font-display text-xs tracking-[0.1rem] font-bold">
                      {status === 'sending' ? 'TRANSMITINDO...' : 'TRANSMITIR DADOS'}
                    </span>
                    {status === 'sending' ? (
         ...
         <section className="w-full lg:w-[320px] flex flex-col gap-8 order-1 lg:order-2">
               ...
               <ul className="flex flex-col gap-4">
                  <li>
                     <a href="#" className="flex items-center gap-4 group">
                        ...
                        <span className="font-display text-[0.65rem] tracking-[0.15rem] text-on-surface-variant group-hover:text-primary transition-colors">TERMINAL_GITHUB</span>
                     </a>
                  </li>
                  <li>
                     <a href="#" className="flex items-center gap-4 group">
                        <div className="w-10 h-10 border-[0.5px] border-outline-variant/50 flex items-center justify-center group-hover:border-[#0A66C2] group-hover:bg-[#0A66C2]/5 transition-colors">
                           <Briefcase size={16} className="text-on-surface-variant group-hover:text-[#0A66C2] transition-colors" />
                        </div>
                        <span className="font-display text-[0.65rem] tracking-[0.15rem] text-on-surface-variant group-hover:text-white transition-colors">REDE_LINKEDIN</span>
                     </a>
                  </li>
                  <li>
                     <a href="#" className="flex items-center gap-4 group">
                        <div className="w-10 h-10 border-[0.5px] border-outline-variant/50 flex items-center justify-center group-hover:border-secondary-container group-hover:bg-secondary-container/5 transition-colors">
                           <MessageCircle size={16} className="text-on-surface-variant group-hover:text-secondary-container transition-colors" />
                        </div>
                        <span className="font-display text-[0.65rem] tracking-[0.15rem] text-on-surface-variant group-hover:text-secondary-container transition-colors">CANAL_DISCORD</span>
                     </a>
                  </li>
                  <li>
                     <a href="#" className="flex items-center gap-4 group">
                        <div className="w-10 h-10 border-[0.5px] border-outline-variant/50 flex items-center justify-center group-hover:border-tertiary-container group-hover:bg-tertiary-container/5 transition-colors">
                           <Mail size={16} className="text-on-surface-variant group-hover:text-tertiary-container transition-colors" />
                        </div>
                        <span className="font-display text-[0.65rem] tracking-[0.15rem] text-on-surface-variant group-hover:text-tertiary-container transition-colors">SINAL_EMAIL</span>
                     </a>
                  </li>
               </ul>
            </div>
            ...
            <div className="border-l-[2px] border-tertiary-container p-4 bg-tertiary-container/10">
               <span className="font-display text-[0.55rem] text-tertiary-container uppercase tracking-[0.1rem] block mb-1">PROTOCOLO_RECEPÇÃO:</span>
               <p className="font-display text-[0.65rem] text-white tracking-[0.1rem] leading-relaxed">
                 TEMPO DE RESPOSTA ESTIMADO: <br />
                 <span className="text-tertiary-container font-bold">&lt; 24 HORAS CICLO TERRESTRE</span>
               </p>
            </div>
         </section>
